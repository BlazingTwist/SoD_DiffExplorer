...
{
	class MenuPropertyEnumSelectionBehavior<T> : IMenuPropertyOnClickBehavior<T>
		where T : struct, Enum
	{
		...
		void IMenuPropertyOnClickBehavior<T>.OnClick(MenuUtils menuUtils, IMenuPropertyAccessor<T> property, string header, int spacing) {
			property.SetValue(Enum.Parse<T>(menuUtils.OpenEnumConfigEditor(header + "." + property.GetFieldName(), property.GetValue().ToString(), Enum.GetNames(typeof(T)), spacing)));
		}
	}
}
